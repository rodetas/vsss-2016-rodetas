...
void Simulator::drawWindow(){
    ...
    textRobots[graphicBall].setString( "Ball: { " + to_string(int(ball.getPosition().x)) + " , " + to_string(int(ball.getPosition().y)) + " }" );
    window.draw(ball);
...
vector<Object> Simulator::getPositions(){
    for (int i = 0; i < robots.size(); i++){
        objects[i].x = robots[i].getPosition().x;
        objects[i].y = robots[i].getPosition().y;
        objects[i].angle = robots[i].getRotation();
    }
    for (int i = robots.size(); i < 6; i++){
        objects[i].x = 0;
        objects[i].y = 0;
        objects[i].angle = 0;
    }
    objects[graphicBall].x = ball.getPosition().x;
    objects[graphicBall].y = ball.getPosition().y;
